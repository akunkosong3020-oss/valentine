...
// --- music controls: autoplay attempt + user-play fallback ---
function updateMusicButton() {
    if (!elements.musicBtn) return;
    elements.musicBtn.setAttribute('aria-pressed', state.musicPlaying ? 'true' : 'false');
    elements.musicBtn.classList.toggle('playing', state.musicPlaying);
    const icon = elements.musicBtn.querySelector('.music-icon');
    if (icon) icon.textContent = state.musicPlaying ? '⏸' : '🔊';
    elements.musicBtn.title = state.musicPlaying ? 'Pause musik' : 'Putar musik';
}
...
function toggleMusic() {
    ...
    if (elements.bgMusic.paused) {
        elements.bgMusic.play().then(() => {
            ...
            // autoplay prevented — user can click the button to start
...
function tryAutoPlayMusic() {
    ...
    elements.bgMusic.volume = 0.65;
    elements.bgMusic.loop = true;
    elements.bgMusic.play().then(() => {
        state.musicPlaying = true;
        updateMusicButton();
    }).catch(() => {
        // Autoplay blocked by browser — leave button so user can start playback
        state.musicPlaying = false;
        updateMusicButton();
    });
}
...
document.addEventListener('DOMContentLoaded', () => {
    // re-bind elements that were added to the DOM
    ...
    // coba autoplay; browser mungkin memblokir — tombol tetap tersedia sebagai fallback
    tryAutoPlayMusic();
});
